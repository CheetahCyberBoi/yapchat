...
impl ChatRoom {
    ...
    pub async fn broadcast_to_clients(&self, msg: Message) {
        ...
        let msg = if let Ok(msg_str) = msg.to_str() {
            // Tack on the timestamp to the message.
            let mut msg_json: serde_json::Value = serde_json::from_str(msg_str).expect("failed to parse message from client");
            msg_json["timestamp"] = serde_json::json!(chrono::Utc::now());
            info!("Appended message with timestamp");
            Message::text(msg_json.to_string())
        } else {
            return;
        };
        ...
        for (&id, client) in self.clients.read().await.iter() {
            if let Err(_e) = client.sender.send(Ok(msg.clone())) {
